...
class SDKRetriever(BaseRetriever):
    ...
    def _get_relevant_documents(self, query: str) -> List[Document]:
        ...
        try:
            ...
            if (not docs) or (max_confidence < self.confidence_threshold and use_web_search):
...
def setup_retriever(dataset_path: str) -> SDKRetriever:
    ...
    web_retriever = WebSearchRetriever() if use_web_search else None
    
    # Build enhanced retriever
    return SDKRetriever(
        vector_store=vector_store,
        web_retriever=web_retriever,
        confidence_threshold=search_confidence_threshold,
        k=fetch_k
    )
